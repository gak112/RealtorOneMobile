...
export class TowerfloorBoxComponent implements OnInit {
  ...
  // TODO: Skipped for migration because:
  //  Your application code writes to the input. This prevents migration.
  readonly copiedFltsData = input<any>(undefined);
  ...
  readonly copiedFlatData = output();
  ...
  async loadFlats() {
    this.flats = await firstValueFrom(
      this.afs
        .collection(
          `ventureTowers/${this.floor().towerId}/floors/${this.floor().id}/flats`,
          (ref) => ref.orderBy('flatNumber')
        )
  ...
  pasteConfiguration() {
    ...
    this.afs
      .doc(`ventureTowers/${this.copiedData().towerId}/floors/${this.floor().id}`)
      .update(updatedVilla)
      .then(async () => {
        this.reloadVilla();
  ...
  async pasteFlatsConfiguration(floor: { id: any }) {
    ...
    if (this.copiedFlat === undefined) {
      ...
      for await (let data of notConfiguredData) {
        ...
        const villaRef = await this.afs.firestore.doc(
          `ventureTowers/${this.floor().towerId}/floors/${floor.id}/flats/${data.id}`
        );
      ...
      const notConfiguredData = await firstValueFrom(
        this.afs
          .collection(
            `ventureTowers/${this.copiedFlat.towerId}/floors/${this.copiedFlat.floorId}/flats`,
            (ref) =>
              ref
                .where('floorId', '==', this.copiedFlat.floorId)
                .where('configured', '==', false)
          )
          .valueChanges({ idField: 'id' })
      );
      ...
      for await (let data of notConfiguredData) {
        ...
        const villaRef = await this.afs.firestore.doc(
          `ventureTowers/${this.floor().towerId}/floors/${this.copiedFlat.floorId}/flats/${data.id}`
        );
        await batch.update(villaRef, Object.assign(updatedVilla));
      }
    }
  ...
  async pasteFlatConfiguration(flat: { floorId: any; id: any }) {
    ...
    if (this.copiedFlat === undefined) {
      ...
      const villaRef = await this.afs.firestore.doc(
        `ventureTowers/${this.floor().towerId}/floors/${flat.floorId}/flats/${flat.id}`
      );
      ...
      const villaRef = await this.afs.firestore.doc(
        `ventureTowers/${this.floor().towerId}/floors/${this.copiedFlat.floorId}/flats/${flat.id}`
      );
      await batch.update(villaRef, Object.assign(updatedFlat));
    }
    await batch
  ...
  async reloadVilla() {
    ...
    this.floor = await firstValueFrom(
      this.afs
        .doc(`ventureTowers/${this.floor().towerId}/floors/${this.floor().id}`)
        .valueChanges()
    );
    this.floor().id = id;
  }
