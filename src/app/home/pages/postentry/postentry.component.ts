...
import { add, videocamOutline, trashOutline, caretDownOutline } from 'ionicons/icons';
import { IonBackButton, IonButton, IonButtons, IonContent, IonHeader, IonIcon, IonImg, IonInput, IonLabel, IonSegment, IonSegmentButton, IonSelect, IonSelectOption, IonTitle, IonToolbar, ModalController } from '@ionic/angular/standalone';
...
@Component({
  ...
  standalone: true,
  imports: [IonHeader,IonToolbar,IonButtons,IonBackButton,IonTitle,IonContent,IonInput,FormsModule,NgIf,IonSelect,IonSelectOption,IonLabel,IonButton,IonIcon,NgFor,IonSegment,IonSegmentButton,UcWidgetModule,IonImg,],
...
export class PostentryComponent implements OnInit {
  ...
  constructor(private modalController: ModalController
    ...
    addIcons({ add, videocamOutline, trashOutline, caretDownOutline})
  }
  ...
  submit() {
    ...
    if (this.request.totalPropertyUnits) {
      if (this.request.totalPropertyUnits.trim() === '') {
        this.toast.showError(
          'Please Select Total Property Units'
        );
        this.loading = false;
        return;
      }
    ...
    if (this.request.propertySizeBuildUp) {
      if (this.request.propertySizeBuildUp === 0) {
        this.toast.showError(
          'Please Enter PropertySize BuildUp'
        );
        this.loading = false;
        return;
      }
